...
const createAndSavePerson = function(data, done) {
  Person.create(data, function(err, person) {
    if (err) return done(err);
    else done(null, person);
  });
  //human.save(function(err, person){
...
var createManyPeople = function(arrayOfPeople, done) {
  Person.create(arrayOfPeople, function(err, people) {
    if (err) {
      ...
      console.log("Multiple People Added to DB");
      done(null, people);
    }
  });
};
...
var findEditThenSave = function(personId, done) {
  ...
  Person.findById({ _id: personId }, function(err, person) {
    ...
    person.favoriteFoods.sort(function(a, b) {
      return a < b;
    });
...
var removeManyPeople = function(len, done) {
